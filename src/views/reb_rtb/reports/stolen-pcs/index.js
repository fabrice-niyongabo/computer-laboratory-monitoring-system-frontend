...
const StolenPcs = () => {
  ...
  const fetchPcs = () => {
    setIsLoading(true);
    Axios.get(BACKEND_URL + "/" + mainPath + "/report/stolen/?token=" + token)
      .then((res) => {
        setTimeout(() => {
          setIsLoading(false);
          ...
          <CCard className="mb-4">
            <CCardHeader>
              <div className="d-flex justify-content-between">
                <div>
                  <strong>Stolen Devices List</strong>
                </div>
                <div className="d-flex justify-content-between">
                  <input
                    type="text"
                  ...
                  <div>&nbsp; &nbsp;</div>
                  <Link
                    to={`/printer/stolen/?stolenDate=${dateFilter}&pcDetails__serialNumber=${keyWord}&description=${keyWord}&pcDetails__model=${keyWord}&sector=${keyWord}&school=${keyWord}&district=${keyWord}`}
                    target="_blank"
                  >
