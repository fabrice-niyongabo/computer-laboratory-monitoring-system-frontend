...
const DamagedPcs = () => {
  ...
  const fetchPcs = () => {
    setIsLoading(true);
    Axios.get(BACKEND_URL + "/" + mainPath + "/report/damaged/?token=" + token)
      .then((res) => {
        setTimeout(() => {
          setIsLoading(false);
          ...
          <CCard className="mb-4">
            <CCardHeader>
              <div className="d-flex justify-content-between">
                <div>
                  <strong>Damaged Devices List</strong>
                </div>
                <div className="d-flex justify-content-between">
                  <input
                    type="text"
                  ...
                  <div>&nbsp; &nbsp;</div>
                  <Link
                    to={`/printer/damaged/?damagedDate=${dateFilter}&pcDetails__serialNumber=${keyWord}&description=${keyWord}&pcDetails__model=${keyWord}&school=${keyWord}&sector=${keyWord}&district=${keyWord}`}
                    target="_blank"
                  >
